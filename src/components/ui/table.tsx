...
function Table({ className, ...props }: React.ComponentProps<'table'>) {
	return (
		<div data-slot='table-container' className='relative w-full overflow-x-auto p-px'>
			<table data-slot='table' className={cn('w-full caption-bottom text-sm border-separate border-spacing-y-1', className)} {...props} />
		</div>
	);
}
...
function TableRow({ className, ...props }: React.ComponentProps<'tr'>) {
	return <tr data-slot='table-row' className={cn('rounded-md', className)} {...props} />;
}
